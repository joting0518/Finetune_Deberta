...
from utils.loss import supervised_contrastive_loss, SimCSE_loss, margin_loss_classification
...
def train_model(args, config, tokenizer, finetune_model, device):
    ...
    if config["model_type"] == "classification":
        ...
        def model_forward_classification(data):
            ...
            m_loss = margin_loss_classification(score, target.long())
            basic_loss = loss_fn(score, target.long())
            # return score, contrastive_or_simCSE_loss, basic_loss, m_loss
            return score, 0, basic_loss, m_loss
        ...
        def model_forward_regression(data):
            ...
            m_loss = margin_loss(score, target)
    ...
    for epoch in range(config['num_epochs']):
        ...
        for data, target in train_loader:
            ...
            score, contrastive_or_simCSE_loss, basic_loss, m_loss = model_forward_classification(data)
            print("target",target.size())
            print("score",score.size())
            total_loss = basic_loss + m_loss + contrastive_or_simCSE_loss
            total_loss.backward()
            optimizer.step()
        ...
        if epoch >= args.start_checkpoint and epoch % args.save_interval == 0:
            checkpoint_path = f"./saved_model/{args.exp_name}_epoch_{epoch}.pth"
    ...
    checkpoint_path = f"./saved_model/{args.exp_name}_final.pth"
    os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
    torch.save(model.state_dict(), checkpoint_path)
